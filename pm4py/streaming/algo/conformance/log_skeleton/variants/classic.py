...
class LogStreamingConformance(StreamingAlgorithm):
    ...
    def update_deviation(self, case, error):
        key = list(error.items())[0][0]
        value = list(error.items())[0][1]
        if case in self.deviations:
            if key in self.deviations[case]:
                self.deviations[case][key].append(value)
            else:
                self.deviations[case][key] = [value]
        else:
            self.deviations[case] = {key: [value]}
    ...
    def verify_directly_follow(self, event):
        ...
        if case in self.last_act_dict:
            ...
            if (act_1, act_2) not in self.log[Outputs.DIRECTLY_FOLLOWS.value]:
                self.message_directly_follows_not_fit(case, act_2, act_1)
    ...
    def verify_frequency(self, event):
        ...
        if case in self.freq_act_dict:
            ...
            if activity in self.freq_act_dict[case]:
                ...
                if act_freq not in self.log[Outputs.ACTIV_FREQ.value][activity]:

                    max_allow = max(self.log[Outputs.ACTIV_FREQ.value][activity])
    ...
    def update_last_act_dict(self, event):
        case = event[self.case_id_key]
        activity = event[self.activity_key]
        self.last_act_dict[case] = activity
