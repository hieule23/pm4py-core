...
def empty_log(log):
    '''Returns bool if log_skeleton is empty'''
    if len(log) == 0:
...
def single_activity(log, activity_key):
    '''Returns bool if log_skeleton consists of single activity only'''
    if log:
        if len(log[0]) >= 1:
            ...
            # check if all traces consist of the same activity, therefore create dfg from log_skeleton and get activities of that dfg
